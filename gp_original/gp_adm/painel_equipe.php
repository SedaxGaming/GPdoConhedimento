			...
			<td align="left" style="padding: 10px 10px 20px 10px;">
				<?php
					...
					if ($fim != '' && $fim < time()) {
						if ($_acesso == '100') {
							if ($status == 1)
								echo "<a href='#' id='confirma' class='btn btn-red'>Confirmar resultado <br> e definir participantes</a>
	...
	<table id="relEquipes" width="100%" class="cellspacing" display="0" width="100%">
		...
		<?php
			...
			$sql = "select 
					...
					e.nome as equipe,
					r.resposta, 
					date_format(r.datahora, '%d/%m/%Y %H:%i:%s') as datahoraF,
					r.datahora,
