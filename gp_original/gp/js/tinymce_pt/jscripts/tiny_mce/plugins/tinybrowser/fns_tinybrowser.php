...
if($level==0 && is_dir($root.$tree.$branch))
	...
	foreach($filetypes as $filetype)
	   {
   	$filenum = $filenum + count(glob($root.$tree.$branch.preg_quote($filetype),GLOB_NOSORT));
   	}
...
while (($dirname = readdir($dh)) !== false)
	...
	if($dirname != '.' && $dirname != '..' && is_dir($root.$tree.$branch.$dirname) && $dirname != '_thumbs')
		...
		foreach($filetypes as $filetype)
		   {
			$filenum = $filenum + count(glob($root.$tree.$branch.$dirname.'/'.preg_quote($filetype),GLOB_NOSORT));
			}
		$indent = '';
